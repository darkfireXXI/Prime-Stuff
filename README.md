...
However this rudimentary check also catches many composite numbers. Assuming we know a couple prime numbers to start (eg. 2, 3, 5) we can then check only these numbers thus reducing the number of verification calculation. Additionally a well known trick is to only check numbers up to the square root of whatever number we are checking as if x factors out to a and b and a<b if we check past sqrt(x) we will find b, which is redundant after finding a.
